...
pub fn get_by_customer(conn: &Connection, customer_id: &str) -> Result<Vec<Kpi>, AppError> {
    let mut stmt = conn.prepare(
        "SELECT id, customer_id, label, value, unit, target, period, updated_at
         FROM kpis WHERE customer_id = ?1 ORDER BY label ASC",
    )?;
    let kpis = stmt
...
pub fn upsert(conn: &Connection, payload: UpsertKpiPayload) -> Result<Kpi, AppError> {
    ...
    conn.execute(
        "INSERT INTO kpis (id, customer_id, label, value, unit, target, period, updated_at)
        ...
        rusqlite::params![
            id, payload.customer_id, payload.label,
    ...
    let kpi = conn.query_row(
        ...
        [&id],
        |row| Ok(Kpi {
            id: row.get(0)?, customer_id: row.get(1)?, label: row.get(2)?,
...
mod tests {
    ...
    fn upsert_updates_kpi_value() {
        ...
        let updated = upsert(&conn, UpsertKpiPayload {
            id: Some(kpi.id), customer_id: "__cynera_privat__".to_string(),
