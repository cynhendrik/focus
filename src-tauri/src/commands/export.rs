...
/// Liefert alle DISTINCT workspace_ids, die in den Kern-Tabellen der lokalen DB
/// vorkommen. Wird beim App-Start zur Selbstheilung verwaister lokaler Workspaces
/// verwendet: fehlende Registrierungen im Frontend werden damit wiederhergestellt.
pub fn list_local_workspace_ids(conn: &rusqlite::Connection) -> Result<Vec<String>, crate::AppError> {
    const TABLES: &[&str] = &[
        "accounts", "invoices", "todos", "crm_follow_ups", "calendar_events", "prepared_items",
    ];
    let mut ids: std::collections::HashSet<String> = std::collections::HashSet::new();
    for table in TABLES {
        // Tabelle existiert ggf. noch nicht (alte Schema-Version) → überspringen.
        let exists: bool = conn
            .query_row(
        ...
        let mut stmt = match conn.prepare(&format!(
            "SELECT DISTINCT workspace_id FROM \"{}\" WHERE workspace_id IS NOT NULL AND workspace_id != ''",
            table
        )) {
            Ok(s) => s,
            Err(_) => continue,
        ...
        let rows = stmt
            ...
            .map_err(|e| crate::AppError::Db(e.to_string()))?;
...
#[tauri::command]
pub fn cmd_list_local_workspace_ids(db: State<'_, DbPool>) -> Result<Vec<String>, crate::AppError> {
    let conn = db.conn();
    list_local_workspace_ids(&conn)
}
...
mod tests {
    ...
    #[test]
    fn list_local_workspace_ids_returns_distinct_ids_from_accounts() {
        let conn = sample_conn();
        // Leere DB → kein Ergebnis.
        let ids = list_local_workspace_ids(&conn).unwrap();
        assert!(ids.is_empty(), "leere DB muss leer zurückgeben");

        // Zwei Zeilen mit workspace_id 'w1', eine mit 'w2'.
        conn.execute(
            "INSERT INTO accounts (id, workspace_id, created_by, name, created_at, updated_at) \
             VALUES ('a1','w1','u','Alpha','2026-01-01','2026-01-01')", []).unwrap();
        conn.execute(
            "INSERT INTO accounts (id, workspace_id, created_by, name, created_at, updated_at) \
             VALUES ('a2','w1','u','Beta','2026-01-01','2026-01-01')", []).unwrap();
        conn.execute(
            "INSERT INTO accounts (id, workspace_id, created_by, name, created_at, updated_at) \
             VALUES ('a3','w2','u','Gamma','2026-01-01','2026-01-01')", []).unwrap();

        let mut ids = list_local_workspace_ids(&conn).unwrap();
        ids.sort();
        assert_eq!(ids, vec!["w1", "w2"], "beide IDs dedupliziert zurück");
    }

    #[test]
    fn list_local_workspace_ids_filters_empty_strings() {
        let conn = sample_conn();
        // Zeile mit leerem workspace_id darf NICHT zurückgegeben werden.
        conn.execute(
            "INSERT INTO accounts (id, workspace_id, created_by, name, created_at, updated_at) \
             VALUES ('a1','','u','Leer','2026-01-01','2026-01-01')", []).unwrap();
        conn.execute(
            "INSERT INTO accounts (id, workspace_id, created_by, name, created_at, updated_at) \
             VALUES ('a2','w1','u','Eins','2026-01-01','2026-01-01')", []).unwrap();
        let ids = list_local_workspace_ids(&conn).unwrap();
        assert_eq!(ids.len(), 1);
        assert_eq!(ids[0], "w1");
    }
