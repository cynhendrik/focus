...
export function WorkspacePicker() {
  ...
  const createLocalWorkspace = useWorkspaceStore(s => s.createLocalWorkspace)
  ...
  const allWorkspaces = [...workspaces, ...localWorkspaces]
    ...
    <div className="h-screen flex items-center justify-center bg-[var(--bg)]">
      <div className="w-full max-w-md p-8">
        ...
        <p className="text-sm text-[var(--text2)] mb-6">
          {allWorkspaces.length === 0 ? 'Erstelle deinen ersten Workspace.' : 'Wähle einen Workspace oder erstelle einen neuen.'}
        </p>
        ...
        <form onSubmit={handleCreate} className="flex flex-col gap-3">
          <p className="text-xs text-[var(--text2)] font-medium">Neuer lokaler Workspace</p>
